...
function load:enter()
    ...
    for i = 1, 3 do
        ...
        local buttonText = hasSave and ("Slot " .. i) or "EMPTY"

        -- Read save date without corrupting pendingData
        local saveDate = nil
        if hasSave then
            -- Read raw file to get just the date, don't use loadGame
            local filePath = save.getFilePath(i)
            local contents = love.filesystem.read(filePath)
            if contents then
                local success, data = pcall(bitser.loads, contents)
                if success and data and data.saveDate then
...
function load:draw()
    ...
    for _, b in ipairs(buttons) do
        ...
        love.graphics.print(b.text, (width - textWidth) / 2, y + 8)

        -- Save date (if any)
        if b.saveDate then
            love.graphics.setFont(smallFont)
            love.graphics.setColor(0.2, 0.2, 0.2, 0.8)
            local dateWidth = smallFont:getWidth(b.saveDate)
            love.graphics.print(b.saveDate, (width - dateWidth) / 2, y + 38)
            -- Reset font and color for next button
            love.graphics.setFont(font)
